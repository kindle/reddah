...
export class SurfacePage implements OnInit {
  ...
  ngOnInit() {
        let currentLocale = null;//this.localStorageService.retrieve("Reddah_Locale");
        let defaultLocale ="en-US"
        if(currentLocale==null){
            if(this.text.isMobile())
            { 
                /*
                this.globalization.getPreferredLanguage()
                .then(res => {
                    //this.localStorageService.store("Reddah_Locale", res.value);
                    this.i18n.loadTranslate(res.value);
                })
                .catch(e => {
                    //this.localStorageService.store("Reddah_Locale", defaultLocale);
                    this.i18n.loadTranslate(defaultLocale);
                });
                */
                this.i18n.loadTranslate(defaultLocale);
            }
  ...
  async signin(){
      ...
      const { data } = await modal.onDidDismiss();
      if(data){
          let result = true//this.authService.exactToken(data);
          if(result){
